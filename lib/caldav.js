    ...
    for (const obj of objects) {
      ...
      try {
        ...
        for (const ve of vevents) {
          ...
          const categoryValues = categories ? categories.getValues() : [];
          const statusProp = ve.getFirstProperty('x-agenda-status');
          const status = statusProp ? String(statusProp.getFirstValue() || '') : '';
          let title = event.summary || '(sans titre)';
          if (categoryValues.length) {
            const suffix = ` (${categoryValues[0]})`;
...
function buildEventICS({ uid, title, start, end, allDay, category, location, status }) {
  ...
  event.uid = uid;
  event.summary = category ? `${title} (${category})` : title;
  if (location) {
  ...
  if (status) {
    vevent.updatePropertyWithValue('x-agenda-status', status);
  }
...
export async function createEvent({ title, start, end, allDay, category, calendarUrl, location, status }) {
  const c = await getClient();
  const calendars = await listCalendars();
  ...
  const ics = buildEventICS({ uid, title, start, end, allDay, category, location, status });
...
export async function updateEvent({ url, etag, uid, title, start, end, allDay, category, location, status }) {
  const c = await getClient();
  const ics = buildEventICS({ uid, title, start, end, allDay, category, location, status });
  const result = await c.updateCalendarObject({
