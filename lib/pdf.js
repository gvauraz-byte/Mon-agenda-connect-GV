...
export function renderAnnualPdf({ res, year, eventsByDay, projects, activeProjectIds, holidaysMap, vacationsMap, filename }) {
  ...
  for (let d = 1; d <= 31; d++) {
    ...
    months.forEach((m, ci) => {
      ...
      const dayEvents = (eventsByDay[dateStr] || []).filter((ev) => {
        ...
        return !proj || activeProjectIds.length === 0 || activeProjectIds.includes(proj.id);
      });
