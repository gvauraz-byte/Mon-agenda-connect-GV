...
// Fin de la journee A PARIS, quelle que soit la saison. Sans ca, un T23:59:59Z
// tombe une heure apres minuit heure francaise et le lien affiche le lendemain.
function finDeJournee(dateStr) {
  const minuitUTC = new Date(`${dateStr}T00:00:00Z`);
  const memeInstantAParis = new Date(
    minuitUTC.toLocaleString('en-US', { timeZone: 'Europe/Paris' })
  );
  const decalage = memeInstantAParis.getTime() - minuitUTC.getTime();
  return new Date(minuitUTC.getTime() + 86400000 - 1 - decalage).toISOString();
}
