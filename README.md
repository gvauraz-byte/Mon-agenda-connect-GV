...
- "Gerer les projets" ouvre un panneau pour renommer un projet, changer sa couleur (touche le rond colore) ou le supprimer, et pour en ajouter un nouveau. L'icone en forme d'oeil (barre ou non) permet d'archiver un projet qui n'est plus d'actualite : il disparait alors de la barre de puces en haut et de la vue annuelle, sans etre supprime — ses evenements passes restent consultables via "Voir la liste", et tu peux le reactiver a tout moment en touchant a nouveau l'oeil.
...
Les outils "Telecharger en PDF", "Liens de partage", "Soumissions" et "Propositions" sont maintenant regroupes dans un seul bouton **"Outils"** (a cote des puces de projets) qui ouvre un petit menu deroulant — moins encombrant que 4 boutons cote a cote. Un chiffre entre parentheses indique s'il y a des soumissions ou propositions en attente.
...
- **Partager** : dans le panneau d'un jour, sous le formulaire d'ajout, remplis un titre (et optionnellement heure/lieu/projet/contact) puis touche le petit bouton **"Partager"**. Un menu s'ouvre avec trois choix : **WhatsApp**, **Texto (SMS)** ou **Email**. Un lien de proposition est cree et l'app de ton choix s'ouvre avec un message pre-rempli (soigne, avec le titre, la date, le lieu) contenant ce lien — il ne te reste qu'a choisir le contact (ou l'envoyer directement si tu as renseigne son numero/email dans le champ "Contact") et appuyer sur envoyer. Le lien lui-meme est court (`.../p/xxxxxxxx` au lieu d'une longue adresse technique), et la page qu'il ouvre est configuree pour qu'un apercu propre (titre du rendez-vous + date) s'affiche dans WhatsApp/Email quand c'est possible, plutot qu'un lien brut. A noter, honnetement : l'adresse de ton app (`...onrender.com`) reste techniquement visible dans le lien lui-meme (WhatsApp ne permet pas de la masquer completement dans un message texte simple) — si tu veux un jour un lien avec ton propre nom de domaine plutot que onrender.com, c'est possible mais ca demande d'acheter un nom de domaine, dis-le moi si ca t'interesse. La personne qui recoit le message ouvre le lien, voit les details, et touche "Accepter" ou "Refuser". Si elle accepte, l'evenement est cree automatiquement dans ton iCloud, sans que tu aies rien a faire. Sur cette meme page, elle peut aussi ajouter le rendez-vous a **son propre agenda** en un clic (telechargement `.ics` compatible Apple/Outlook, ou lien direct vers Google Calendar) — ce n'est pas lie a ta reponse "Accepter/Refuser", c'est juste pratique pour elle. Le panneau "Propositions" (dans "Outils", avec un compteur si des propositions attendent une reponse) liste toutes tes propositions envoyees et leur statut (en attente / acceptee / refusee), avec la possibilite de les supprimer. A noter : le choix WhatsApp n'est pas une integration officielle WhatsApp Business (qui demande une validation par Meta et n'est pas gratuite) — c'est le lien "click-to-chat" public de WhatsApp (`wa.me`), donc c'est toujours toi qui appuies sur "Envoyer" dans WhatsApp, l'app ne peut pas l'envoyer toute seule a ta place. Meme chose pour le texto et l'email : l'app prepare le message, c'est toi qui l'envoies.
